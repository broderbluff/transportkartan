...
//     if (isReversed) {
//       points = [...originalPoints.reversed];
//     } else {
...
//     double angle = 0.0;
//     if (_previousPoint != null)    angle = -atan2(thisPoint.latitude - _previousPoint?.latitude, thisPoint.longitude - _previousPoint?.longitude) - 4.7128;
...
//         point: thisPoint, angle: _lastAngle, animValue: animValue, controllerValue: controllerValue, builtPoints: builtPoints);
//   }
...
//     double distance =
//         2 * earthRadius * asin(sqrt(pow(sin(lat2 - lat1) / 2, 2) + cos(lat1) * cos(lat2) * pow(sin(lon2 - lon1) / 2, 2)));
...
//     animation = Tween<double>(begin: widget.begin, end: interpolator.totalDistance)
//         .animate(CurvedAnimation(parent: controller, curve: Curves.easeInOut))
//       ..addListener(() {
//         InterpolatedResult interpolatedResult =
//             interpolator.interpolate(controller.value, animation.value, widget.interpolateBetweenPoints);

//         /// not sure we need a tween at this point anymore, controller only ?

//         widget.duringCallback
//             ?.call(interpolatedResult.builtPoints, interpolatedResult.point, interpolatedResult.angle, animation.value);
//       })
...
//         (oldWidget.originalPoints != widget.originalPoints || (oldWidget.isReversed != widget.isReversed))) {
